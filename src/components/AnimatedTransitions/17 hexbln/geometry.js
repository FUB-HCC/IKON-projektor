  ...
  getHighestX(){
    ...
    if (this.getLength() == 0)
      ...
      this.vertices.forEach(function(d,i){
        if (max == undefined || d.pos.x > max.pos.x || d.pos.x == max.pos.x && d.pos.y > max.pos.y // nimmt den obersten, wenn mehrere die gleiche x-Pos haben
        ) {
          max = d;
          idx = i;
        }
...
class Cluster {
  ...
  makeHull2Path(xScale, yScale){
    return this.getHullVertices()
      .map(function(hull){return hull.makeHull2Path(xScale, yScale)})
      .join(" ");
  }
