...
class Polygon {
  ...
  createTransTable(newDatas) {
    // this.vertices = oldDatas
    // newNodes = [Knoten]
    ////////// vergleicht alte und neue Knoten
    var changes = {};
    var oldDatas = this.vertices;
    var matchFound, clNr, node;
    var i = 0, j = 0;
    while (i < oldDatas.length){
      matchFound = false;
      while (j < newDatas.length) {
        if (oldDatas[i].equal(newDatas[j])) {
          clNr = oldDatas[i].clusterNo;
          if (changes[clNr] == undefined)
            changes[clNr] = {"old": [], "new": []};
          changes[clNr]["old"].push(oldDatas[i]);
          clNr = newDatas[j].clusterNo;
          if (changes[clNr] == undefined)
            changes[clNr] = {"old": [], "new": []};
          changes[clNr]["new"].push(newDatas[j]);
          newDatas[j].clusterNo = oldDatas[i].clusterNo;
          oldDatas.splice(i,1);
          newDatas.splice(j,1);
          matchFound = true;
        }
    ...
    // verbliebene Knoten müssen übernommen werden
    oldDatas.forEach(function(d){
      console.log(clNr);
      if (changes[clNr] == undefined)
        changes[clNr] = {"old": [], "new": []};
      changes[clNr]["old"].push(d);
    });
    newDatas.forEach(function(d){
      clNr = d.clusterNo;
      if (changes[clNr] == undefined)
        changes[clNr] = {"old": [], "new": []};
      changes[clNr]["new"].push(d);
    });
    
    // erstellt Polygone und teilt diese in Cluster ein (Nester)
    var values;
    for (i in changes) {// values = {"old": _, "new": _}
      values = changes[i];
      if (values.old.length > 0){
        values.old = new Nest(values.old);
      }
      if (values.new.length > 0)
        values.new = new Nest(values.new);
    } // changes = {k1: {"old": N1, "new": N2}, k2: ...}
      // k1 = clusterNo, N1,N2 = Nest
    
    // modifiziert alte Nester
    for (i in changes) {
      if (changes[i].old.length == 0)// neues Cluster hinzu
        changes[i].old = changes[i].new.copy();
      else if (changes[i].new.length == 0){// ein Cluster wird gelöscht
        changes[i].new = undefined;
      }
      else {// old, new != []
        var polygons = [], id, verts;
        var newCluster = changes[i].new.flatNestToCluster(-1);
        for (j in changes[i].new.nest) {
          id = changes[i].new.nest[j].id;
          // modifiziert old
          verts = changes[id].old.nest
            .filter(function(c){return c.id == id;})
            [0].polygon.vertices;
          polygons = polygons.concat(verts);
          // modifiziert new
          changes[i].new.nest[j] = newCluster.copy();
          changes[i].new.nest[j].id = parseInt(id);
        }
        changes[i].old = new Nest(polygons);
      }
    }
    
    // füllt ggf. alte und neue Hüllen auf
    var huellen, oldHull, newHull;
    for (i in changes) {
      if (changes[i].new != undefined && 
        changes[i].new.length > 0 && changes[i].old.length > 0)
        for (j in changes[i].new.nest) {
          oldHull = changes[i].old.nest[j].getHullVertices();
          newHull = changes[i].new.nest[j].getHullVertices();
          huellen = oldHull.huellenAbgleichen(newHull); // [P0,P1]
          changes[i].old.nest[j] = huellen[0];
          changes[i].new.nest[j] = huellen[1];
        }
    }
     ...
     * new: Nest[0: C3, 1: C4, ...]
     * }}
     * nun muss nur noch je eine Transition von jedem Nestelement zum jeweils anderen gemacht werden. z.B.: C1 -> C3, sofern diese die gleiche id haben.
     */
  }
  
  static filterTransTable(table, oldNew){
    var newTable = {};
    for (i in table) {
      if (table[i][oldNew] != undefined) {
        newTable[i] = table[i][oldNew];
      }
    }
    return newTable;
  }
...
class Nest {// [Cluster_1,... , Cluster_n]
  ...
  flatNestToCluster(clusterNo){// vereinigt alle Cluster zu einem
    ...
    this.nest.forEach(function(c){
      vertices = vertices.concat(c.polygon.vertices);
    });
  ...
  makeHull2Path(scale){
    ...
    this.nest.forEach(function(cl){
      path = path + cl.getHullVertices().makeHull2Path(scale);
    });
    return path;
  }
} // Ende: Klasse Nest
