...
Promise.all([
  ...
  if (window.File && window.FileReader && window.FileList && window.Blob) {
    ...
    pfade = new Pfade (oldNests, svg, "huelle", scale);
      pfade.label.style("opacity", 0);
      pfade.hull.style("opacity", 0);
...
//         
//         var pfade = new Pfade (oldNests, svg, "huelle", scale);
//         pfade.hull//svg.svg.select("g.hulls").selectAll("path.huelle")
//           .style('opacity', 0)
//           .transition().duration(2000)
//           .style('opacity', hullOpacity);
//         pfade.label//svg.svg.select("g.beschriftung").selectAll("text.huelle")
//           .style('opacity', 0)
//           .transition().duration(2000)
//           .style('opacity', 1);
//         
//         var weiterBtn = new LinkButton(me, storeDatas, +1, "Zur nächsten Aufgabe ⊳", "");
//       }
//     }, "Eingabe bestätigen");
  } else {
...
function callAusgangszustand(){
  ausgangszustand([null, svg], oldDataset, oldNests, scale);
}
