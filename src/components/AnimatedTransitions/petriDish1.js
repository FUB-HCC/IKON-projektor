...
function update(oldDatas, oldNests){// (oldDatas, oldNests, delay)
  ...
  ////////////////////// Hüllen //////////////////////
  var neueNester = new Nest(getFilteredData(dataset)).nest;
  ...
  var abgleich = [];
  oldNests.forEach(function(cluster){
    // modifiziert die Cluster des Nests, Daten sind bereits gefiltert
    var oldHull, newHull;
    oldHull = cluster.copy().getHullVertices();
    var nestX = neueNester.find(function(d){
      return d.id == cluster.id
    });
    
    if (nestX != undefined) {// es ex. weiterhin eine Hülle gl. ID
      newHull = nestX.getHullVertices(); // sind Polygone
      var huellen = oldHull.huellenAbgleichen(newHull);
      oldHull = huellen[0];
      newHull = huellen[1];
    }
    abgleich.push({clusterNo: cluster.id, oldHull: oldHull, newHull: newHull});
  });
  
  //console.log(abgleich);
  
  var hulls = svg.svg.select("g.hulls").selectAll("path.class42")
    .data(neueNester, function(d){return d.id;});
