...
class PetriDishView extends Component {
  constructor(props) {
    ...
    for (let i = 0; i < dataDump.project_data.length; i++) {
      projects.push({
        point: dataDump.project_data[i].point,
        cluster: dataDump.project_data[i].cluster,
        projectId: dataDump.project_data[i].id
      });
    }
    this.state = {
  ...
  updateHulls() {
    ...
    this.state.nlpProjectsDump.forEach(value => {
      if (clusters[value.cluster]) {
        clusters[value.cluster].push([
          value.point[0] * scale + offsetX,
          value.point[1] * scale + offsetY
        ]);
    ...
    clusters.forEach((points, index) => {
      let clusterId = index;
      if (points.length >= 3) {
        // d3.polygonHull requires at least 3 data points
        ...
        let hull = svg
          ...
          .on("mouseenter", e => {
            ...
            let clusterPathElement = document.getElementById(
              `cluster-hull-${clusterId}`
            );
            let clusterPoints = document.getElementsByClassName(
              `cluster-point-${clusterId}`
            );
            let n = 0;
            while (n < clusterPoints.length / 2) {
              // workaround for bringing all data points and the hull to the foreground
              for (let i = 0; i < clusterPoints.length; i++) {
                document
                  .getElementById("cluster-svg")
                  .appendChild(clusterPoints[i]);
              }
              document
                .getElementById("cluster-svg")
                .insertBefore(clusterPathElement, clusterPoints[0]);
              n++;
            }
          })
          .on("mousemove", points => {
