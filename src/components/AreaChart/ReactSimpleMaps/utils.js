...
export function createNewChildren (children, props) {
  ...
  if (!children.length) {
    ...
    }) : (isChildOfType(children, ['Group', 'Markers', 'Lines', 'Trees', 'Annotations', 'Annotation', 'Graticule'])
    ...
    return children.map((child, i) => {
      ...
      return isChildOfType(child, ['Geographies'])
        ? React.cloneElement(child, {
          ...
          projection: props.projection
        }) : (isChildOfType(child, ['Group', 'Markers', 'Lines', 'Trees', 'Annotations', 'Annotation', 'Graticule'])
          ? React.cloneElement(child, {
            key: `zoomable-child-${i}`,
            projection: props.projection,
            zoom: props.zoom,
