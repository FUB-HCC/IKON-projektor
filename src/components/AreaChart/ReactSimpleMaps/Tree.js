...
class Tree extends Component {
  ...
  mouseOutNode(d) {
    const { onNodeMouseOut, projection } = this.props;
  ...
  mouseEnterNode(d) {
    if (d.parent) {
      const { onNodeMouseEnter, projection } = this.props;
      d.coordinates = projection.invert([d.x, d.y]);
      this.setState({ hoveredNode: d });
      return (
        onNodeMouseEnter && onNodeMouseEnter(d, projection.invert([d.x, d.y]))
      );
    } else {
      return false;
    }
  ...
  clickNode(d) {
    const { onNodeClick, projection } = this.props;
    d.coordinates = projection.invert([d.x, d.y]);
  ...
  render() {
    ...
    let treemap = d3.tree().size([width * 0.8, height]);
    ...
    g.selectAll(".link")
      ...
      .attr("d", d => {
        ...
        if (!d.parent) {
          ...
          dy = height - treeY;
          if (d.data.i % 2 !== 0) {
            // results in two lines of projects
            dy = height - treeY - 30;
          }
        }
    ...
    node
      ...
      .style("fill", d => {
        if (
          ...
          d.data.id === this.state.selectedNode.data.id &&
          d.parent
        )
          return "#623336";
        if (
          this.state.hoveredNode &&
          d.data.id === this.state.hoveredNode.data.id &&
          d.parent
        )
          return "#623336";
    ...
    return <g className={"rsm-tree"} id={"institution-tree"} />;
  }
}
