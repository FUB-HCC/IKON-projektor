...
class FilterModal extends Component {
  constructor(props) {
    ...
    expFilters.fill(false);
    this.state = { expandedFilters: expFilters, value: "" };
    this.handleFilterExpand = this.handleFilterExpand.bind(this);
    this.handleInputChange = this.handleInputChange.bind(this);
  }

  handleFilterExpand(key) {
    const newState = this.state.expandedFilters.slice();
    newState[key] = !newState[key];
    this.setState({ expandedFilters: newState });
  }
  ...
  render() {
    const filterElements = this.props.filters.map((filter, key) =>
      filter.type === "a" ? (
        <FilterElement
          id={key}
          key={key}
          name={filter.name}
          keys={filter.distValues}
          value={filter.value}
          open={this.state.expandedFilters[key]}
          change={this.props.filterChangeHandler}
          expand={this.handleFilterExpand}
        />
      ) : null
    );
    return (
      <div className={classes.FilterModal}>
        ...
        <div className={classes.closebutton} onClick={this.props.closeModal}>
          <img src={CloseIcon} />
        </div>
      </div>
    );
