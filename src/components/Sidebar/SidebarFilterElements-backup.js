...
const filterElement = (props) => {
  const filters = props.keys.map((k, key) => {
    ...
    console.log(props.value, 'props.value.some(v => v === k)')
    ...
    <div style={{height: props.open ? expandedHeight : '70px'}} className={classes.FilterElement}>
      <div className={classes.Title}>
        <div className={classes.Text}>
          ...
          <div className={classes.Indicator}><div className={classes.IndicatorDigit}>{props.value.length}</div></div>
        </div>
        <svg cursor='pointer' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64' className={props.open ? classes.ButtonExpanded : classes.ButtonCollapsed} onClick={() => props.expand(props.id)}>
