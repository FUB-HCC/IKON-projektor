      ...
      <div className={classes.FirstBlock}>
        <div className={classes.UpperHead}>
          <CheckBox
            ...
            disable={true}
            checked={false}
            name={filters[0].name.toUpperCase()}
        ...
        <div>
          {filters[0].subGroup.map((subGroup, sgIndex) => {
            const color =
              getTopicColor(subGroup.name) === "#989aa1"
                ? getFieldColor(subGroup.name)
                : getTopicColor(subGroup.name);
            ...
            return (
              <div className={classes.CheckContainer} key={sgIndex}>
                <CheckBox
                  ...
                  disable={false}
                  checked={true}
                  name={subGroup.name.toUpperCase()}
                  classes={classes.subGroup}
                  color={color}
                  change={props.change}
                  id={"forschungsgebiet"}
                  value={subGroup.name}
                  ...
                  {subGroup.values.map((name, nIndex) => {
                    return (
                      <CheckBox
                        ...
                        color={color}
                        change={props.change}
                        id={sgIndex === 3 ? "forschungsgebiet" : "hauptthema"}
                        value={name}
                      />
                    );
                  })}
      ...
      <div className={classes.FirstBlock}>
        <div className={classes.UpperHead}>
          <CheckBox
            ...
            disable={true}
            checked={false}
            name={filters[1].name.toUpperCase()}
            classes={classes.Title}
          />
        </div>
        <div className={`${classes.CheckContainer} ${classes.groupMarginFix}`}>
          {filters[1].values.map((item, index) => (
            <CheckBox
              checkColor={classes.grey}
              show={true}
              disable={false}
              key={index}
              checked={true}
...
class CheckBox extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <span className={props.classes} style={{ color: props.color }}>
          {props.name}
        </span>
        <input
          {...config}
          ...
          type="checkbox"
          id={props.name}
          onChange={() => this.onChange(props.id, props.name, "a")}
        />
        {props.show && (
          <label
            className={classes.CheckBoxLabel + " " + props.checkColor}
            htmlFor={props.name}
          />
        )}
      </div>
